...
Scene: split or clearly contrasted composition, before state and after state, product visible in the after. Where the angle is about what she uses today, the before state shows the baseline alternative from the one-pager's "Compared to" section in use (the lesser version in the same category, e.g. plastic utensils), never a competitor's product. TEXT OVERLAY: "Before" / "After" labels plus optionally one short Stage 2 line. NEGATIVE RULES additionally forbid implying the before state is dangerous, harmful, or disgusting, and forbid exaggerated unrealistic outcomes.

7 — IMAGE TYPE: Comparison image
Scene: side-by-side layout against the baseline alternative from the one-pager's "Compared to" section (the lesser version in the same category: plastic utensils beside the wooden ones, a still bowl beside the fountain), never a competitor brand's product. Product side cleaner and more desirable; the baseline side shows the limitation named by the strongest "Compared to" criterion, without becoming cartoonish. TEXT OVERLAY: product name label and "[baseline alternative]" label plus optionally one short comparison line from Stage 2. NEGATIVE RULES additionally forbid implying the alternative is unsafe, harmful, dirty, or medically inferior.
