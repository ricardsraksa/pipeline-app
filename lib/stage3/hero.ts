...
Scene: ONE split composition of the SAME space from the SAME camera angle. LEFT HALF: the messy, chaotic everyday situation the customer knows, WITHOUT the product — e.g. for a closet organizer, a cramped closet with clothes piled and crushed together; for a pill organizer, loose bottles crowding a counter. Real and relatable, not staged disaster. RIGHT HALF: the same space transformed, with the product visibly in use and clearly the reason it is fixed — tidy, calm, easy to take in. The product appears ONLY on the right. Lighting on the right may be slightly warmer/brighter, but both halves must stay photorealistic and belong to one image. TEXT OVERLAY: one short problem label over the left half and one short product-led fix line from Stage 2 over the right half. NEGATIVE RULES additionally forbid making the left side disgusting, dirty, dangerous, or shameful, and forbid exaggerated unrealistic transformations.
