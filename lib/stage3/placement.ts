...
/** Runs placement and stores it. Throws PlacementError(400) when there is nothing to place. */
export async function runPlacement(runId: number): Promise<StoredPlacement> {
  const run = await getRun(Number(runId));
  if (!run) throw new PlacementError("Run not found", 404);
  ...
  if (usable.length < 2) {
    throw new PlacementError("Need at least 2 finished images to place into sections", 400);
  }
