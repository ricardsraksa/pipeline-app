...
async function migrateDB() {
  const newColumns = [
    ...
    // One shared context for the run (lib/run-context.ts): every operator edit
    // in order, and the fingerprints each generated stage was built from.
    "run_edits TEXT",
    "angles_built_on TEXT",
    ...
    "stage2_json_at TEXT",
    // What this run has uploaded to each weekly ads folder: JSON
...
export interface Run {
  ...
  run_edits: string | null;
  angles_built_on: string | null;
