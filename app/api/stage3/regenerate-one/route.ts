...
import { regenerateOneImage } from "@/lib/stage3/jobs";

// One image: generate, audit and store server-side in a single call, so a tab
// that closes mid-way loses nothing. ~40-90s.
...
export async function POST(req: NextRequest) {
  ...
  try {
    const image = await regenerateOneImage(body.runId, body.index, typeof body.prompt === "string" ? body.prompt : undefined, refs);
