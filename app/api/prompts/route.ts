...
import { PRODUCT_PROMPT, STAGE1_PROMPT, ANGLES_PROMPT, STAGE2_PROMPT, STAGE3_PROMPT, getPrompt, ADS_PROMPT } from "@/lib/prompts";
import {
  loadPromptsFile,
  writePromptsFile,
  getHistory,
...
const STAGES: PromptStage[] = ["product", "stage1", "angles", "stage2", "stage3", "ads"];

export async function GET(req: Request) {
  const denied = requireSession(req); if (denied) return denied;
  const data = await loadPromptsFile();
  const [product, stage1, angles, stage2, stage3, ads] = await Promise.all([
    getPrompt("product"),
    getPrompt("stage1"),
    getPrompt("angles"),
    getPrompt("stage2"),
  ...
  return Response.json({
    ...
    saved_at: {
      ...
      ads: getCurrentOverride(data, "ads")?.saved_at ?? null,
    },
    history: {
